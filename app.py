...
file3 = st.sidebar.file_uploader("3. ไฟล์บันทึกพลังงานและแผนผลิต (Energy & Plan)", type=["csv", "xlsx", "xls"])
file4 = st.sidebar.file_uploader("4. ไฟล์ยอดส่งมอบจริง (Actual Production)", type=["csv", "xlsx", "xls"])
...
if file1 and file2 and file3 and file4:
    ...
    df_energy = read_uploaded_file(file3)  # ไฟล์ที่ 3 เก็บค่า Plan
    df_actual = read_uploaded_file(file4)  # ไฟล์ที่ 4 เก็บค่า Actual
    
    # -------------------------------------------------------------------------
    # 🛠️ ส่วนดึงข้อมูลและคำนวณจากคอลัมน์จริงที่คุณระบุ
    # -------------------------------------------------------------------------
    
    # 1. ดึงยอด Plan จากไฟล์ 3 คอลัมน์ "จำนวน(ตัน)"
    try:
        total_plan = float(pd.to_numeric(df_energy['จำนวน(ตัน)'], errors='coerce').sum())
    except Exception as e:
        st.sidebar.error("❌ หาคอลัมน์ 'จำนวน(ตัน)' ในไฟล์ที่ 3 ไม่พบ")
        total_plan = 450.0  # fallback

    # 2. ดึงยอด Actual จากไฟล์ 4 คอลัมน์ "mton"
    try:
        total_actual = float(pd.to_numeric(df_actual['mton'], errors='coerce').sum())
    except Exception as e:
        st.sidebar.error("❌ หาคอลัมน์ 'mton' ในไฟล์ที่ 4 ไม่พบ")
        total_actual = 0.0

    # 3. ดึงนาที Downtime สะสมจากไฟล์ที่ 1 (ดึงคอลัมน์ตัวเลขคอลัมน์แรก)
    try:
        total_downtime = int(df_downtime.select_dtypes(include=[np.number]).sum().iloc[0])
    except:
        total_downtime = 0

    # 4. ประมวลผลตารางวัตถุดิบและ Yield Loss จากไฟล์ที่ 2
    try:
    ...
    # 6. ดัชนีพลังงานไฟฟ้าจำเพาะ (SEC)
    try:
        # พยายามดึงตัวเลขการใช้ไฟฟ้ารวมจากไฟล์ที่ 3 (ถ้ามีตัวเลขช่องอื่นจะดึงช่องแรกมาคำนวณ)
        total_kwh = float(df_energy.select_dtypes(include=[np.number]).sum().iloc[0])
    except:
        total_kwh = 40500
    sec_kwh_per_ton = total_kwh / max(1.0, total_actual)
    
    # โครงสร้างสำหรับกดเซฟรายเดือน
    monthly_summary = {
        "Month": month_name,
        "OEE": round(oee, 2),
    ...
    with tabs[0]:
        ...
        col2.metric("ปริมาณแร่ที่ผลิตได้จริง (mton)", f"{total_actual:,.1f} ตัน", delta=f"{total_actual-total_plan:,.1f} ตัน จากแผน")
        ...
        <div class="card">
            ...
            ในเดือน <b>{month_name}</b> นี้ โรงงานทำ OEE ได้ {oee:.1f}% โดยมียอดผลิตจริงอยู่ที่ {total_actual:,.1f} ตัน จากแผนการผลิต {total_plan:,.1f} ตัน 
            จุดวิกฤตที่ควรปรับปรุงคือการลดเวลาหยุดเครื่องจักร (Downtime) เพื่อผลักดันให้ยอดการบดและการส่งมอบแร่เป็นไปตามเป้าหมายของแผนในคลังระบบ ERP
        </div>
        """, unsafe_allow_html=True)
    ...
    with tabs[4]:
        ...
        with col_btn1:
            if st.button("🔥 บันทึกข้อมูลของเดือนนี้เข้าสู่ระบบประวัติกลาง (Save Data)", use_container_width=True):
                ...
                st.success(f"ระบบเซฟข้อมูลของเดือน {month_name} เรียบร้อยแล้ว! กราฟด้านล่างจะอัปเดตเพิ่มทันที")
